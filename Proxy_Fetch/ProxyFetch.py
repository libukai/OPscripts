...
# @Last Modified time: 2016-07-10 21:57:48
...
def get_proxy(url_list):
    ...
    for proxy_detail in proxy_rows:
        ip_address = proxy_detail[1].text
        ip_port = proxy_detail[2].text
        ip_type = proxy_detail[5].text.lower()
        proxies_list.append((ip_type, '{0}://{1}:{2}'.format(ip_type, ip_address, ip_port)))
    return proxies_list

def check_proxy(proxy_all):
...
if __name__ == '__main__':
    ...
    url_list = ['http://www.xicidaili.com/wn/' + str(i) for i in range(1,2)]
